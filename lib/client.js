    ...
    function apply(ctx) {
      ...
      function applyWidth(node) {
        ...
        try {
          ...
          if (w > 0) {
            node.style.width = Math.max(WIN_MIN_WIDTH, w - SIDEBAR_GAP - SIDEBAR_GAP) + "px";
            return w;
          }
        } catch (err) { /* ignore */ }
        return -1;
      ...
      // 稳定性以「侧边栏原始列宽」为准而非钳制后的窗口宽度：收起→展开动画期间
      // 原始列宽每帧都在变化，不会因钳制在最小宽度而误判稳定、提前停止。
      function followWidth() {
        if (winNode === null || frameNode === null) { stopFollow(); return; }
        const raw = applyWidth(winNode);
        if (raw === lastRaw) {
          stableFrames += 1;
      ...
      // （重新）启动宽度跟随：重置计数与记录值，避免收起/展开后沿用旧值导致误停
            ...
            try {
              frameWaitObs = new MutationObserver(() => {
                ...
                if (frameNode !== null && frameWaitObs !== null) {
                  ...
                  frameWaitObs = null;
                  if (winNode !== null) { applyCollapsed(); startFollow(); }
                  startFrameObs();
                }
              });
