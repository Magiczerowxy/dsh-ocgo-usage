    ...
    var dswSessionClip = (function () {
      ...
      function clipInit(m) {
        ...
        m.update = function () {
          ...
          var lineTop = Math.max(0, top - LINE_GAP);
          lineEl.style.display = "block";
          lineEl.style.top = lineTop + "px";
          lineEl.style.width = info.width + "px";
          var lists = clipScrollContainers();
          var next = [];
          for (var j = 0; j < lists.length; j++) {
            try {
              var listTop = lists[j].getBoundingClientRect().top;
              var maxH = lineTop - listTop - LIST_GAP;
              if (maxH >= MIN_CAP) {
                lists[j].style.maxHeight = maxH + "px";
                clipApplyFade(lists[j]);
                clipApplyClamp(lists[j], lineTop);
                next.push(lists[j]);
              }
            } catch (err) { /* ignore */ }
          }
          for (var k = 0; k < m.capped.length; k++) {
            if (next.indexOf(m.capped[k]) === -1) {
              try { m.capped[k].style.maxHeight = ""; } catch (err) { /* ignore */ }
              try { m.capped[k].style.webkitMaskImage = ""; } catch (err) { /* ignore */ }
              try { m.capped[k].style.maskImage = ""; } catch (err) { /* ignore */ }
              try { m.capped[k].style.clipPath = ""; } catch (err) { /* ignore */ }
              try { m.capped[k].style.webkitClipPath = ""; } catch (err) { /* ignore */ }
            }
          }
          m.capped = next;
        };
      ...
      function clipEnsure() {
        ...
        if (m === undefined) m = window.__dswSessionClip__ = { refs: 0, capped: [] };
      ...
      function clipRelease() {
        ...
        m.line = null; m.capped = []; m.raf = null;
