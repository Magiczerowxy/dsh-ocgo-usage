window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var dswSessionClip = (function () {
      ...
      var LIST_GAP = 6;  // 会话列表底部到分界线的留白
      var MIN_CAP = 72;  // 可用高度过小（异常布局）时不裁剪，避免破坏布局
      var FADE_H = 40;    // 淡化主区起点（距列表底部）
      var FADE_MID = 30;  // 淡化中间档（60% 透明度）
      var FADE_SOFT = 24; // 淡化尾部（15% 透明度）
      var FADE_END = 16;  // 完全透明起始高度：此高度到列表底边之间无任何内容残留
      var SELECTORS = [".ds-win", ".ds-win-chip", ".ds-oc", ".ds-oc-chip"];
      ...
      function clipInit(m) {
        ...
        // 逐帧插值动画：纯写入路径（上面 applyPositions），动画期间不再触发任何
        // 强制同步布局，消除下降/上升时的卡顿；收敛后做一次完整重测
        m.animStep = function () {
          m.animRaf = null;
          var target = m.targetTop;
          var next = m.animTop + (target - m.animTop) * 0.18;
          if (Math.abs(target - next) < 0.5) next = target;
          var moved = Math.abs(target - next) > 0.5;
          m.animTop = next;
          try { m.applyPositions(); } catch (err) { /* ignore */ }
          if (moved) {
        ...
        m.update = function () {
          ...
          if (top === null || info === null) {
            if (lineEl !== null && lineEl !== undefined) lineEl.style.display = "none";
            if (m.animRaf !== null && m.animRaf !== undefined) { try { window.cancelAnimationFrame(m.animRaf); } catch (err) { /* ignore */ } m.animRaf = null; }
            m.visible = false;
            m.animTop = undefined;
        ...
        try {
          ...
          m.bodyObs.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ["class", "style"] });
        } catch (err) { /* ignore */ }
        try { m.update(); } catch (err) { /* ignore */ }
      }
      ...
      function clipEnsure() {
        ...
        if (m === undefined) m = window.__dswSessionClip__ = { refs: 0, capped: [], lastCalcs: [] };
      ...
      function clipRelease() {
        ...
        m.lastInfo = undefined; m.lastCalcs = [];
